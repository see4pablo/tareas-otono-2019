...
filtered_grouped = FILTER results_grouped BY time <= 3000 AND time > 0;
...
pablo_average = CROSS filtered_grouped, total_average;
...
results_person = JOIN pablo_average BY person, persons_cut BY id;
...
average_per_country = FOREACH results_by_country GENERATE group AS countryId, AVG(results_person.time) AS time_country, COUNT(results_person) AS nPerson, AVG(results_person.totalAverage) as totalAverage;
...
final_results = FOREACH results_country GENERATE name, time_country, nPerson , (time_country + (totalAverage*100/nPerson)) as pablo_score;
...
STORE results_ordered INTO '/uhadoop2019/elsueco/results_project8/';
